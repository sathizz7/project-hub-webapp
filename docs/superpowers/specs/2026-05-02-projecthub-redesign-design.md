...
- **2026-05-03 (later):** Plan 8 (Final Cleanup) merged on `feature/final-cleanup` — AI Capture page rebuilt with real `CaptureSession`/`CaptureItem` DB + Claude parsing via `POST /api/capture/process`; `/projects/new` wired to real `/api/users` + `POST /api/projects`; `/calendar` stubbed (full calendar deferred); `/team/manage` rebuilt as Prisma user roster. All 10 `_archive/` files deleted. `mock-data.ts` and `ai-capture-parser.ts` deleted. 138 tests across 25 files. **Redesign complete.**
- **2026-05-03 (later):** Plan 7 (Team Section) merged on `feature/team-section` — replaces legacy team pages with Server Component–backed pages: Team Overview with `MemberCard` grid (performance score, active projects, avg response), department filter pills, sort controls; Member Profile with 3-tab layout (Overview with projects + submissions, Tasks, Leaves); Leave & Availability with 7-day week strip and inline pending leave approve/reject via `PATCH /api/leave-requests/[id]`. Pure helpers (`deriveDepartment`, `filterMembers`, `sortMembers`, `buildWeekStrip`) unit tested. 133 tests across 24 files.
- **2026-05-03 (later):** Plan 6 (Review Queue + Projects List) merged on `feature/reviews-and-projects` — replaces legacy mock-data pages with Server Component–backed pages: Review Queue with date grouping (Today/Yesterday/Earlier), type-filter tabs (All/Code/Docs/Demos), Sheet drawer with AI-assisted feedback via `/api/ai/review` + `/api/feedback`; Projects List with type/status filters and priority/deadline/progress sort, reusing `ProjectCard`. Pure helpers (`groupByDate`, `matchesTypeFilter`, `filterProjects`, `sortProjects`) unit tested. 107 tests across 22 files.
